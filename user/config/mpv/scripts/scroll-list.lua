...
local mp = require 'mp'
local scroll_list = {
    -- NOTE: colors follow the format BBGGRR
    -- this means that a hex value of #123456
    -- would become 563412
    global_style = [[]],
    -- color_4
    list_style = [[{\q2\fs25\c&He9ded8&}]],
    -- color_8
    header_style = [[{\q2\fs35\c&Hd0c088&}]],
    wrapper_style = [[{\fs16\c&Hd0c088&}]],
    cursor_style = [[{\c&Hd0c088&}]],
    selected_style = [[{\c&Hd0c088&}]],
...
function scroll_list:update_ass()
    ...
    if #self.list < 1 then
        self:append(self.wrapper_style)
...
function scroll_list:insert(item) self.list[#self.list] = item end
...
function scroll_list:new()
    ...
    vars = {
        ...
        keybinds = {
            {
                'j', 'scroll_down', function() vars:scroll_down() end,
                {repeatable = true}
            },
            {
                'k', 'scroll_up', function() vars:scroll_up() end,
                {repeatable = true}
            }, {'q', 'close_browser', function() vars:close() end, {}}
        }
    }
